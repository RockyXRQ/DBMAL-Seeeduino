...
int step = 1;           // 颜色渐变步长
int db = 0;             // 声强传感器虚拟值
int mirage_choice = 0;  // 幻彩方案选择变量
int music_choice = 0;   // 音乐方案选择变量
...
void Strip_Update_Fade(int step);      // 灯条颜色更新(渐变)
void Strip_Update();                   // 灯条颜色更新(立即)
void Strip_Close();                    // 关灯
void Strip_Solid();                    // 常亮照明模式
void Strip_Mirage(int mm_choice = 0);  // 幻彩氛围模式
void Strip_Music(int mm_choice = 0, int db_step = 1024);  // 音乐模式

// 幻彩&音乐相关函数
void MM_Dusk();                    // 夕阳西下模式
void MM_Disco(int db_step = 103);  // 蹦迪模式
void MM_Fantasy(int db = 1024);    // 梦幻模式
...
#ifdef USE_INTERRUPT
    ITimer.attachInterruptInterval(SYSTEM_INTERVAL_MS * 1000, DBMAL_Event);
#endif
#if DEBUG_MODE
    Serial.begin(115200);
#endif
...
// 灯条颜色更新(渐变)
void Strip_Update_Fade(int step) {
    ...
    for (int i = 0; i < NUM_STRIPS; i++) {
        for (int j = 0; j < NUM_LEDS_PER_STRIP; j++) {
            leds[i][j].r +=
                LIMIT((int)((int)leds_target[i][j].r - (int)leds[i][j].r),
                      (-1 * step), step);
            leds[i][j].g +=
                LIMIT((int)((int)leds_target[i][j].g - (int)leds[i][j].g),
                      (-1 * step), step);
            leds[i][j].b +=
                LIMIT((int)((int)leds_target[i][j].b - (int)leds[i][j].b),
                      (-1 * step), step);
        }
    }
}
...
// 幻彩氛围模式
void Strip_Mirage(int mm_choice) {
    switch (mm_choice) {
        case 0:
            MM_Dusk();
            break;
...
// 音乐模式
void Strip_Music(int mm_choice, int db_step) {
    switch (mm_choice) {
        case 0:
            MM_Disco(db_step);
            break;
